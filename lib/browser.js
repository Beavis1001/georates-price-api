...
const { findRoomPrice, parseAmount, extractExclusiveTaxPct, extractAbsoluteExtraTax, normalizeCurrency, detectSessionCurrency, listRooms, enrichRoomOptions } = require('./parser');
...
async function attemptFetch(targetUrl, proxyServer, proxyAuth, blockScripts, device) {
  ...
  try {
    ...
    return { bodyText, rooms: roomData, roomMeta, loadedOk, transferBytes, finalUrl, err: null };
  } catch (err) {
...
async function fetchPrice(countryCode, targetUrl, proxyServer, userPrefix, password, room, board, cancel, rates, maxAttempts, device) {
  ...
  const [rawAmt, ctx, curTok, geniusAbzug, stornoArt, verpflegungArt, deals] = findRoomPrice(bodyText, room, board, cancel);
