...
const { findRoomPrice, findRoomPriceMobile, parseAmount, extractExclusiveTaxPct, extractAbsoluteExtraTax, extractFixedFees, normalizeCurrency, detectSessionCurrency, listRooms, enrichRoomOptions } = require('./parser');
...
// Erwachsene und Naechte aus dem Booking-Link (fuer Belegung und Abgaben "pro Nacht").
// Ohne Angabe nimmt Booking selbst 2 Erwachsene an.
function sucheAusLink(link) {
  try {
    const u = new URL(link);
    const adults = parseInt(u.searchParams.get('group_adults') || u.searchParams.get('req_adults') || '2', 10) || 2;
    const ci = u.searchParams.get('checkin');
    const co = u.searchParams.get('checkout');
    const nights = ci && co ? Math.round((Date.parse(co) - Date.parse(ci)) / 86400000) : 0;
    return { adults, nights: nights > 0 && nights < 90 ? nights : 0 };
  } catch (e) { return { adults: 2, nights: 0 }; }
}
...
async function fetchPrice(countryCode, targetUrl, proxyServer, userPrefix, password, room, board, cancel, rates, maxAttempts, device) {
  ...
  const suche = sucheAusLink(targetUrl);
  const [rawAmt, ctx, curTok, geniusAbzug, stornoArt, verpflegungArt, deals] = mobil
    ? findRoomPriceMobile(bodyText, room, board, cancel, { adults: suche.adults })
    : findRoomPrice(bodyText, room, board, cancel, { adults: suche.adults });
  ...
  if (rawAmt) {
    ...
    // Feste Abgaben ("€ 15 Umweltabgabe pro Nacht", "€ 159.03 Steuer pro Aufenthalt") kommen
    // ZUSAETZLICH zu den Prozenten oben und stehen oft in einer anderen Waehrung als der Preis
    // (Unterkunft EUR, Sitzung USD). Deshalb ueber Euro umrechnen und dann in beide Werte.
    let abgabenEuro = 0;
    const abgabenText = [];
    // Steht schon ein Gesamtbetrag "zzgl. X Steuern und Gebuehren" da (absExtra), ist die Abgabe
    // darin enthalten - nicht doppelt zaehlen.
    for (const f of (val !== null && absExtra === null ? extractFixedFees(ctx) : [])) {
      const feeCur = normalizeCurrency(f.curTok, currency);
      const feeRate = rates[feeCur];
      if (!feeRate) continue;
      const faktor = f.einheit === 'aufenthalt' ? 1 : (f.einheit === 'person_nacht' ? suche.nights * suche.adults : suche.nights);
      if (!(faktor > 0)) continue;
      abgabenEuro += f.amount * faktor * feeRate;
      abgabenText.push(`${f.amount} ${feeCur}${f.einheit === 'aufenthalt' ? '' : ' x ' + faktor}`);
    }
    ...
    if (val !== null) {
      ...
      if (rate) {
        if (abgabenEuro > 0) {
          val = Math.round((val + abgabenEuro / rate) * 100) / 100;
          result.priceLocal = val;
          result.priceRaw += ` | zzgl. Abgaben ${abgabenText.join(' + ')} -> ${val} ${currency}`;
        }
        result.priceEuro = Math.round(val * rate * 100) / 100;
      }
    }
  } else {
