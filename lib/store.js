...
// Gesamtzaehler fuer die Startseite ("X Preis-Checks bisher", seit 25.09.2026). Gezaehlt wird jede
// Suche mit Ergebnis, frisch oder aus dem Cache - fuer den Besucher ist beides ein Check. Die
// Suchen VOR dem Zaehler stehen als SUCHEN_BASIS in api/best-of.js.
const SUCHEN_KEY = 'georates:suchen:gesamt';
async function sucheZaehlen() {
  if (!upstash()) return;
  await cmd('incr', SUCHEN_KEY);
}
async function suchenGesamt() {
  const n = parseInt(await cmd('get', SUCHEN_KEY), 10);
...
module.exports = {
  ...
  logQuery,
  bestofSpeichern, bestofLesen, sucheZaehlen, suchenGesamt,
