...
// Ausgangsland = das Land, aus dem der Besucher gerade surft (Vercel-Header x-vercel-ip-country),
// seit 25.09.2026. Vorher kam es nur aus der Sprachendung des Links, und die kannte bloss Laender
// aus der festen Liste: Wer aus Frankreich, Spanien, Italien, den Niederlanden, UK, Oesterreich
// oder der Schweiz kam, wurde gegen DEUTSCHLAND verglichen - sein eigener Preis wurde nie gemessen.
// Das Herkunftsland ist genau die Sitzung, deren Preis der Besucher selbst sieht (auch im Urlaub
// oder mit VPN). Nur Laender mit bekannter Waehrung; Gebiete ueber Alias/Mutterland wie beim
// Hotelland. Ohne Header (Tests, Debug) oder unbekanntes Land: wie bisher aus dem Link.
function ausgangslandFuerBesucher(besucherLand, link) {
  let land = String(besucherLand || '').trim().toUpperCase();
  land = HOTEL_LAND_ALIAS[land] || land;
  land = HOTEL_LAND_MUTTERLAND[land] || land;
  // Tuerkei nicht: von dort sind keine internationalen Buchungen moeglich (siehe Waehrungstabelle).
  if (/^[A-Z]{2}$/.test(land) && land !== 'TR' && DEFAULT_CURRENCY_BY_COUNTRY[land]) return land;
  return detectBaselineCountry(link);
}
...
function laenderFuerDieseSuche(link, auswahl, baseline) {
  ...
  // Ausgangsland ausserhalb der festen Liste (z.B. Frankreich, Schweiz): ganz nach vorn.
  if (baseline && !liste.includes(baseline)) liste = [baseline, ...liste];
...
module.exports = {
  ...
  ausgangslandFuerBesucher, hotelLandAusLink, hotelNameAusLink, hotelNameAusTitel, proxyLandFuerHotel, laenderFuerDieseSuche,
