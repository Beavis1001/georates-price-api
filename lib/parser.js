...
//
// UNGEPRUEFT gegen die echte Seite - diese Funktion ist aus dem Screenshot abgeleitet. Deshalb
// laeuft der Mobil-Abruf nur mit MOBILE_CHECK=1, als eigene Zeile neben dem Desktop-Ergebnis, und
// der Handler verwirft alles, was ausserhalb von 50 bis 105 % des Desktop-Preises liegt
// (`mobilBewerten`). Zum Nachziehen: Debug-Abruf mit device=android und debugLines, dann die
// Muster hier gegen den echten Text anpassen und einen Ausschnitt (erfundene Namen!) als Fixture
// in test/parser-test.js legen.
  ...
  while (k < maxEnd) {
    ...
    if (MOBILE_TOTAL_RE.test(lines[k])) {
      ...
      for (; j < Math.min(k + 4, maxEnd); j++) {
        ...
        if (MOBILE_STRIKE_HINT_RE.test(lines[j])) continue; // "Originalpreis € X Aktueller Preis € Y"
        break;
      }
      if (amount) {
