  ...
  while (k < maxEnd) {
    ...
    if (MOBILE_TOTAL_RE.test(lines[k])) {
      ...
      if (amount) {
        ...
        stufen.push({ amount, cur, anchor: k, blockStart, ctx, genius: null, deals: dealsInZeilen(lines, blockStart, end + 1) });
        blockStart = end + 1;
        k = end;
      }
    }
...
function findRoomPriceMobile(bodyText, roomName, boardType, cancelPref) {
  const lines = bodyText.split('\n').map((l) => l.trim()).filter(Boolean);
  const roomLower = roomName.toLowerCase();
  const starts = [];
  for (let i = 0; i < lines.length; i++) {
...
module.exports = {
  ...
  findRoomPrice, findRoomPriceMobile, waehleStufe, mobilBewerten, normalizeCurrency, detectSessionCurrency, implausibleVsBaseline, summarize,
