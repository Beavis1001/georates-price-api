...
const DEAL_MUSTER = [
  // "Preis nur fuer Mobilgeraetnutzer" ist Bookings Wortlaut auf der mobilen Seite (21.09.2026).
  { key: 'mobile', re: /mobile[- ]?(rate|preis|deal|angebot)|mobil(er|e)?[- ]?(rate|preis|rabatt)|mobilger(?:ä|ae)t/i },
  { key: 'online_payment', re: /online[- ]?zahlung|bei online-zahlung|booking\.com bezahlt|zahl(en|ung) (jetzt|online).*(rabatt|sparen)/i },
  // "Genius Rabatt koennte verfuegbar sein": Booking kuendigt Genius nur an ("Genius" ist ein Bild,
  // im Text steht "Rabatt koennte verfuegbar sein"). Abgezogen wird erst eingeloggt im Checkout -
  // und nicht in jedem Land: Rodos Park, 24.09.2026, deutsche Sitzung ohne, US-Sitzung mit
  // Genius (-9,6 % Endpreis). Bis dahin landete die Zeile als allgemeiner "Deal".
  { key: 'genius_hint', re: /rabatt k(?:ö|oe)nnte verf(?:ü|ue)gbar/i },
...
// Feste Abgaben aus "Nicht inbegriffen: ...". Anlass 24.09.2026, Rodos Park (Griechenland), US-Sitzung:
//   "Nicht inbegriffen: 13 % Mehrwertsteuer, € 15 Umweltabgabe pro Nacht, 0,5 % Übernachtungssteuer"
//   "Nicht inbegriffen: € 159.03 Steuer pro Aufenthalt"            (Partnerangebot)
// Bis dahin wurden nur die Prozente aufgeschlagen; die 90 EUR Umweltabgabe fehlten, und das
// US-Ergebnis war um genau diesen Betrag zu billig (Scheinvorsprung). Die Abgabe steht in der
// Waehrung der UNTERKUNFT (hier EUR), der Zimmerpreis in der der Sitzung (USD) - deshalb gibt
// diese Funktion Betrag UND Waehrungszeichen zurueck, umgerechnet wird in browser.js.
// "Inbegriffen: € 89.83 Resortgebühr" bleibt aussen vor: schon im Preis.
const FEE_TOKEN_RE = /(US\$|[A-Z]{3}|[^\d\s,.%:]{1,3})\s?(\d[\d.,]*)\s+[^,\d]*?\bpro\s+(Person\s+(?:und|pro)\s+Nacht|Nacht|Aufenthalt)/gi;
function parseFeeAmount(t) {
  const s = String(t || '').trim();
  // Englische Schreibweise "159.03" (so steht es in Partnerangeboten) - parseAmount lese 15903.
  if (/^\d+\.\d{1,2}$/.test(s)) return parseFloat(s);
  return parseAmount(s);
}
function extractFixedFees(context) {
  const text = String(context || '');
  const i = text.search(/nicht inbegriffen/i);
  if (i < 0) return [];
  // Bis zu einer "Inbegriffen:"-Angabe oder hoechstens vier Zeilen weit lesen.
  let seg = text.slice(i).split('\n').slice(0, 4).join(' ');
  const inkl = seg.slice(18).search(/(^|\s)inbegriffen:/i);
  if (inkl >= 0) seg = seg.slice(0, 18 + inkl);
  const out = [];
  for (const m of seg.matchAll(FEE_TOKEN_RE)) {
    const amount = parseFeeAmount(m[2]);
    if (amount === null || !(amount > 0)) continue;
    const einheit = /aufenthalt/i.test(m[3]) ? 'aufenthalt' : (/person/i.test(m[3]) ? 'person_nacht' : 'nacht');
    out.push({ amount, curTok: m[1], einheit });
  }
  return out;
}

// Belegung eines Tarifs: "max. Personenzahl: 1" / "Nur für 1 Gast" steht im Block VOR dem Preis.
// Anlass 24.09.2026: Mit "egal = guenstigster Tarif" gewann beim Rodos Park der Tarif fuer EINE
// Person (2.007 statt 2.271 EUR) - die Suche war fuer zwei. Liefert null, wenn nichts dasteht.
const BELEGUNG_RE = /max\.?\s*Personen(?:zahl)?:?\s*(\d+)|Nur f(?:ü|ue)r\s+(\d+)\s+Gast/i;
function belegungIn(lines, von, bis) {
  let wert = null;
  for (let i = Math.max(0, von); i <= bis && i < lines.length; i++) {
    const m = BELEGUNG_RE.exec(lines[i]);
    if (m) wert = parseInt(m[1] || m[2], 10);
  }
  return wert;
}
function passendeBelegung(stufen, adults) {
  if (!adults) return stufen;
  const passend = stufen.filter((t) => t.maxPersonen == null || t.maxPersonen >= adults);
  return passend.length ? passend : stufen;
}
...
function tarifstufen(lines, start, maxEnd) {
  ...
  while (k < maxEnd) {
    ...
    // Zimmergrenze: Nach "Zimmer auswaehlen" beginnt entweder der naechste Tarif (Belegung,
    // dann Preis) oder das naechste ZIMMER (Name, Betten, Beschreibung, "24 m²"). Steht ein
    // "m²" vor dem naechsten Betrag, ist es ein neues Zimmer. Ohne diese Grenze sammelte die Liste
    // Tarife des Folgezimmers ein - harmlos, solange die erste passende Stufe gewann, gefaehrlich,
    // seit bei "egal" die guenstigste gewinnt (24.09.2026).
    if (rawTiers.length && /^Zimmer ausw(?:ä|ae)hlen$/i.test(lines[k])) {
      let neuesZimmer = false;
      for (let j = k + 1; j < Math.min(k + 12, maxEnd); j++) {
        if (PRICE_PREFIX_RE.test(lines[j]) || AMOUNT_LINE_RE.test(lines[j])) break;
        if (lines[j].includes('m²')) { neuesZimmer = true; break; }
      }
      if (neuesZimmer) break;
    }
  ...
  const dealsIm = (von, bis) => {
    ...
    if (gefunden.has('genius_hint')) gefunden.delete('genius'); // nur angekuendigt, nicht abgezogen
  ...
  return rawTiers.map((t, i) => {
    ...
    return {
      ...
      maxPersonen: belegungIn(lines, t.blockStart, t.anchor),
    };
  });
}

function findRoomPrice(bodyText, roomName, boardType, cancelPref, opts) {
  ...
  const tiers = passendeBelegung(stufen, opts && opts.adults).map((t) => [t.amount, t.ctx, t.cur, t.genius, t.deals || []]);
  return waehleStufe(tiers, boardType, cancelPref);
}

// Aus den Tarifstufen eines Zimmers die waehlen, die Verpflegung und Storno des Nutzers am
// besten trifft. Seit 21.09.2026 eigene Funktion: Der Mobil-Parser (unten) zerlegt die Seite
// anders, muss aber nach GENAU denselben Regeln auswaehlen - sonst vergleicht man am Ende
// einen Smartphone-Tarif ohne Fruehstueck mit einem Desktop-Tarif mit Fruehstueck.
function waehleStufe(tiers, boardType, cancelPref) {
  ...
  // Innerhalb einer Prioritaetsstufe gilt der GUENSTIGSTE passende Tarif, nicht der erste auf der
  // Seite. Betreiber-Entscheidung vom 24.09.2026: "egal" heisst "guenstigster". Bis dahin nahm das
  // Tool den ersten Treffer; das war meist auch der billigste, aber nicht bei Rabatten auf
  // einzelnen Karten: Am Handy lag eine "Flexibel"-Karte mit Mobile Rate unter der
  // "Preiswert"-Karte darueber. Gleichstand -> der erste bleibt (strikt kleiner).
  const guenstigste = (liste) => {
    let best = null;
    let bestWert = Infinity;
    for (const t of liste) {
      const w = parseAmount(t[0]);
      if (w === null) { if (!best) best = t; continue; }
      if (w < bestWert) { best = t; bestWert = w; }
    }
    return best;
  };
  const stufen = [
    // 1. Verpflegung passt UND Storno passt ausdruecklich
    (t) => matchesBoard(t[1]) && stornoAusdruecklichPasst(t[1]),
    // 2. Storno passt ausdruecklich, Verpflegung passt oder steht gar nicht dabei
    (t) => (matchesBoard(t[1]) || !hatBoardInfo(t[1])) && stornoAusdruecklichPasst(t[1]),
    // 3./4./5. erst beides locker, dann Verpflegung, dann Storno
    (t) => matchesBoard(t[1]) && matchesCancel(t[1]),
    (t) => matchesBoard(t[1]),
    (t) => matchesCancel(t[1]),
  ];
  for (const passt of stufen) {
    const g = guenstigste(tiers.filter(passt));
    if (g) return mitTarifart(g);
  }
  return mitTarifart(guenstigste(tiers));
}

// ---- Mobiles Layout (Experiment MOBILE_CHECK, seit 21.09.2026) ------------------------------
// Booking zeigt Smartphones eigene Tarife ("Preis nur fuer Mobilgeraetnutzer"), im beobachteten
// Fall 10 % unter dem Desktop-Preis ALLER 16 Laender. Das mobile Layout ist im Text anders
// aufgebaut als das Desktop-Layout, deshalb versteht `tarifstufen` es nicht (Messung 17.09.:
// Filterbezeichnungen als Zimmer, kein "Preis € X" in einer Zeile, keine Zeile "Einschliesslich
// Steuern und Gebuehren" direkt unter dem Preis).
//
// Beobachtete Struktur einer mobilen Tarifkarte (Screenshot 21.09.2026, deutsche Sitzung):
//   <Tarifname, z. B. "Preiswert + Fruehstueck">
//   Preis fuer:                      <- Belegungssymbole
//   Sehr gutes Fruehstueck im Preis inbegriffen
//   Nicht kostenlos stornierbar
//   Online-Zahlung
//   Preis nur fuer Mobilgeraetnutzer  <- Deal-Plakette
//   Preis fuer 5 Naechte:             <- ANKER dieser Stufe
//   € 1.899                          <- durchgestrichener Originalpreis (optional)
//   € 1.709                          <- tatsaechlicher Preis = LETZTE Betragszeile nach dem Anker
//   Es koennen zusaetzliche Gebuehren anfallen.
//   Reservieren                      <- Ende der Karte
// Die Karten eines Zimmers stehen unter dessen Ueberschrift. Der Anker ist die Zeile "Preis fuer
// N Nacht/Naechte", weil sie in jeder Karte genau einmal vorkommt.
//
// Am 24.09.2026 gegen echte mobile Seiten geprueft (Android-Kennung, deutsche Sitzung, zwei
// Berliner Hotels, 19 Tarifkarten). Zwei Formen kommen vor:
//   ohne Rabatt:  "Preis fuer 2 Naechte:" / "€ 269" / "Preis € 269" / "Einschliesslich Steuern ..."
//   Mobile Rate:  "Preis fuer 2 Naechte:" / "€ 246 € 188" / "Originalpreis € 246 Aktueller Preis € 188"
// Streichpreis und Preis stehen bei der Mobile Rate in EINER Zeile. Die erste Fassung (aus einem
// Screenshot abgeleitet) erwartete sie auf zwei Zeilen und fand deshalb genau die Handy-Rabatte
// nie. Preise sind inklusive Steuern und Gebuehren. Der Handler verwirft trotzdem alles
// ausserhalb von 50 bis 150 % des Desktop-Preises (`mobilBewerten`).
const MOBILE_TOTAL_RE = /^Preis f(?:ü|ue)r \d+ N(?:ä|ae)cht(?:e)?:?$/i;
const MOBILE_CARD_END_RE = /^(Reservieren|Zimmer ausw(?:ä|ae)hlen|Jetzt reservieren)$/i;
const MOBILE_STRIKE_HINT_RE = /Originalpreis|Aktueller Preis/i;
// "€ 246 € 188": Streichpreis und Preis in einer Zeile, gleiche Waehrung - der zweite gilt.
const MOBILE_ZWEI_BETRAEGE_RE = /^([^\d\s]{1,6})\s*([\d][\d.,]*)\s+\1\s*([\d][\d.,]*)\s*$/;
// "Originalpreis € 246 Aktueller Preis € 188": der Betrag hinter "Aktueller Preis" gilt.
const MOBILE_AKTUELL_RE = /Aktueller Preis\s*([^\d\s]{1,6})\s*([\d][\d.,]*)/i;

function mobileTarifstufen(lines, start, maxEnd) {
  const stufen = [];
  let blockStart = start + 1;
  let k = start + 1;
  while (k < maxEnd) {
    if (stufen.length && looksLikeNewRoomHeading(lines, k)) break;
    if (MOBILE_TOTAL_RE.test(lines[k])) {
      // Nach dem Anker bis zu drei Betragszeilen; die letzte ist der Preis, den man zahlt.
      // Steht nur eine da, gibt es keinen Streichpreis.
      let amount = null;
      let cur = null;
      let j = k + 1;
      for (; j < Math.min(k + 4, maxEnd); j++) {
        const m = AMOUNT_LINE_RE.exec(lines[j]);
        if (m) { amount = m[2]; cur = m[1]; continue; }
        const zwei = MOBILE_ZWEI_BETRAEGE_RE.exec(lines[j]);
        if (zwei) { amount = zwei[3]; cur = zwei[1]; continue; }
        const aktuell = MOBILE_AKTUELL_RE.exec(lines[j]);
        if (aktuell) { amount = aktuell[2]; cur = aktuell[1]; continue; }
        if (MOBILE_STRIKE_HINT_RE.test(lines[j])) continue;
        break;
      }
      if (amount) {
        // Kartenende suchen: "Reservieren" oder die naechste Karte / das naechste Zimmer.
        let end = j;
        while (end < Math.min(j + 6, maxEnd) && !MOBILE_CARD_END_RE.test(lines[end]) && !MOBILE_TOTAL_RE.test(lines[end])) end++;
        const ctx = lines.slice(blockStart, Math.min(end + 1, maxEnd)).join('\n');
        stufen.push({ amount, cur, anchor: k, blockStart, ctx, genius: null, deals: dealsInZeilen(lines, blockStart, end + 1), maxPersonen: belegungIn(lines, blockStart, k) });
        blockStart = end + 1;
        k = end;
      }
    }
...
// Deal-Plaketten in einem Zeilenbereich (gleiche Regeln wie in tarifstufen).
function dealsInZeilen(lines, von, bis) {
  const gefunden = new Set();
  for (let i = Math.max(0, von); i < Math.min(bis, lines.length); i++) {
    for (const d of DEAL_MUSTER) if (d.re.test(lines[i])) gefunden.add(d.key);
  }
  if (gefunden.has('genius_hint')) gefunden.delete('genius');
  if (gefunden.size > 1) gefunden.delete('deal');
  return [...gefunden];
}

// Gegenstueck zu findRoomPrice fuer das mobile Layout. Gleiche Rueckgabe (7 Elemente), gleiche
// Stufenauswahl. Findet die Seite keine mobile Tarifkarte unter dem Zimmernamen, kommt
// [null, null] zurueck - NIE ein geratener Desktop-Wert.
function findRoomPriceMobile(bodyText, roomName, boardType, cancelPref, opts) {
  const lines = bodyText.split('\n').map((l) => l.trim()).filter(Boolean);
  const roomLower = roomName.toLowerCase();
  const starts = [];
  for (let i = 0; i < lines.length; i++) {
  ...
  const stufen = [];
  for (const s of starts) {
    for (const t of mobileTarifstufen(lines, s, Math.min(s + 250, lines.length))) {
      if (gesehen.has(t.anchor)) continue;
      gesehen.add(t.anchor);
      stufen.push(t);
    }
...
// Smartphone-Zeile gegen das Desktop-Ergebnis bewerten. Konservativ wie bei den Laendern:
// Ersparnis gegen den niedrigeren Ausgangspreis (baselineUsedEuro, also ggf. den Nutzerpreis),
// Schwelle wie fuer ein Land in derselben Waehrung. Ein Smartphone-Preis, der weit unter oder
// ueber dem Desktop-Preis liegt, ist mit hoher Wahrscheinlichkeit ein Parserfehler (anderes
// Zimmer, Preis pro Nacht statt Aufenthalt) und wird verworfen - lieber "nicht gelesen" als
// eine falsche Zahl, die plausibel aussieht.
const MOBILE_MIN_FACTOR = 0.5;
// Obergrenze seit 24.09.2026 bei 150 % statt 105 %: Rodos Park, Handy 2.271 EUR ohne den
// Online-Zahlungs-Rabatt, Desktop 2.117,60 mit - also 107 %, ein echter Preis. Er wurde als
// Lesefehler verworfen und die Zeile zeigte "–". Ein HOEHERER Handy-Preis kann niemanden zu einer
// falschen Buchung verleiten (er ist nie ein Fund), die Untergrenze schuetzt vor dem gefaehrlichen
// Fall: einem falsch gelesenen Schnaeppchen.
const MOBILE_MAX_FACTOR = 1.5;
// Seit 24.09.2026: Jede Stichprobe wird EINZELN geprueft, und die Obergrenze richtet sich nach dem
// HOECHSTEN Ausgangspreis-Abruf. Anlass, echter Lauf: Handy 188 / 209 EUR (Mobile Rate kam nur in
// einer Sitzung), Desktop 209,10 / 209,10 / 194,56. Zusammengefuehrt auf 209 lag das Handy ueber
// 105 % des NIEDRIGSTEN Desktop-Preises und wurde als Lesefehler ausgeblendet ("–"), obwohl es
// schlicht ein schwankender Rabatt war. Jetzt: 209 bleibt der vorsichtige Preis, dazu
// `schwankt` und `bestSeenEuro`, damit das Frontend "nicht in jeder Sitzung" sagen kann.
function mobilBewerten(mobileRow, summary) {
  if (!mobileRow) return null;
  const out = {
    device: mobileRow.device || 'Smartphone', country: mobileRow.country,
    priceEuro: mobileRow.priceEuro != null ? mobileRow.priceEuro : null,
    priceLocal: mobileRow.priceLocal != null ? mobileRow.priceLocal : null,
    currency: mobileRow.currency || null, priceRaw: mobileRow.priceRaw || null,
    deals: Array.isArray(mobileRow.deals) ? mobileRow.deals : [],
    samples: Array.isArray(mobileRow.samples) ? mobileRow.samples : (mobileRow.priceEuro != null ? [mobileRow.priceEuro] : []),
    spreadPct: mobileRow.spreadPct != null ? mobileRow.spreadPct : null,
    savingsPct: null, relevant: false, implausible: false, beatsBestCountry: false,
    schwankt: false, bestSeenEuro: null, bestSeenSavingsPct: null,
  };
  const basis = summary && summary.success && summary.baselineUsedEuro != null ? summary.baselineUsedEuro : null;
  if (basis == null) return out;
  const basisWerte = [basis, ...((summary.baselineSamples || []).filter((v) => v != null))];
  const basisMax = Math.max(...basisWerte);
  const plausibel = (v) => v != null && v >= basis * MOBILE_MIN_FACTOR && v <= basisMax * MOBILE_MAX_FACTOR;
  const gueltig = out.samples.filter(plausibel);
  if (!gueltig.length) {
    if (out.priceEuro == null && !out.samples.some((v) => v != null)) return out;
    out.implausible = true;
    out.priceEuro = null;
    out.priceLocal = null;
    out.priceRaw = 'Smartphone-Preis nicht verlässlich erkannt';
    return out;
  }
  // Vorsichtig: der hoechste plausible Handy-Preis gilt (wie beim Siegerland).
  const vorsichtig = Math.max(...gueltig);
  if (out.priceEuro !== vorsichtig) { out.priceEuro = vorsichtig; out.priceLocal = out.currency === 'EUR' ? vorsichtig : null; }
  out.savingsPct = Math.round(((basis - out.priceEuro) / basis) * 1000) / 10;
  out.relevant = out.savingsPct >= RELEVANT_SAVINGS_PCT;
  const bestCountryEuro = summary.best && summary.best.priceEuro != null ? summary.best.priceEuro : null;
  out.beatsBestCountry = out.relevant && (bestCountryEuro == null || out.priceEuro < bestCountryEuro);
  const minG = Math.min(...gueltig);
  out.schwankt = gueltig.length >= 2 && (vorsichtig - minG) / minG >= RELEVANT_SAVINGS_PCT / 100;
  if (out.schwankt) {
    out.bestSeenEuro = minG;
    out.bestSeenSavingsPct = Math.round(((basis - minG) / basis) * 1000) / 10;
  }
  return out;
}
...
module.exports = {
  ...
  findRoomPrice, findRoomPriceMobile, waehleStufe, extractFixedFees, parseFeeAmount, mobilBewerten, normalizeCurrency, detectSessionCurrency, implausibleVsBaseline, summarize,
